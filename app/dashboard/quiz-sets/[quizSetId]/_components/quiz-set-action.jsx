  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex items-center gap-x-2">
        <Button className="rounded-md" variant="outline" size="sm" onClick={() => setAction("change-active")}>
          {published ? "Unpublish" : "Publish"}
        </Button>

        <Button className="rounded-md" type="submit" name="action" value="delete" size="sm" onClick={() => setAction("delete")}>
          <Trash className="h-4 w-4 rounded-md" />
        </Button>
      </div>
    </form>
  );
